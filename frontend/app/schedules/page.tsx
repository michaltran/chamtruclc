                                    ...
                                    {canEdit && (
                                      <div className="hidden group-hover:flex gap-0.5 ml-1 shrink-0">
                                        {s.status==='draft' && isAdmin && (
                                          <button onClick={()=>handleApprove(s.id)} className="text-green-600 hover:text-green-800">✓</button>
                                        )}
                                        <button onClick={()=>handleDelete(s.id)} className="text-red-400 hover:text-red-600">✕</button>
                                      </div>
                                    )}
                                  </div>
                                ))}
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              <div>
                ...
                <select value={form.userId} onChange={e=>setForm({...form,userId:e.target.value})}
                  ...
                  {users.map(u=><option key={u.id} value={u.id}>{u.fullName} ({u.title||u.department?.name||''})</option>)}
                </select>
              </div>
              <div>
